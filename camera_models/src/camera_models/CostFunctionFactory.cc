...
void worldToCameraTransform(const T *const q_cam_odo, const T *const t_cam_odo, const T *const p_odo,
                            const T *const att_odo, T *q, T *t, bool optimize_cam_odo_z = true)
{
...
public:
    ...
    ReprojectionError3(const std::vector<double> &intrinsic_params, const Eigen::Vector3d &odo_pos,
                       const Eigen::Vector3d &odo_att, const Eigen::Vector2d &observed_p, bool optimize_cam_odo_z) :
        m_intrinsic_params(intrinsic_params),
        m_odo_pos(odo_pos),
        m_odo_att(odo_att),
        m_observed_p(observed_p),
    ...
    ReprojectionError3(const std::vector<double> &intrinsic_params, const Eigen::Quaterniond &cam_odo_q,
...
ceres::CostFunction *CostFunctionFactory::generateCostFunction(const CameraConstPtr &camera, const Eigen::Vector3d &odo_pos,
                ...
                case Camera::PINHOLE_FULL:
                    if (optimize_cam_odo_z) {
                        costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<PinholeFullCamera>, 2, 4, 3, 3>(
                            new ReprojectionError3<PinholeFullCamera>(intrinsic_params, odo_pos, odo_att, observed_p,
                                                                      optimize_cam_odo_z));
                    } else {
                ...
                case Camera::SCARAMUZZA:
                    if (optimize_cam_odo_z) {
                        costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<OCAMCamera>, 2, 4, 3, 3>(
                            new ReprojectionError3<OCAMCamera>(intrinsic_params, odo_pos, odo_att, observed_p,
                                                               optimize_cam_odo_z));
                    } else {
...
ceres::CostFunction *CostFunctionFactory::generateCostFunction(
    const CameraConstPtr &camera, const Eigen::Quaterniond &cam_odo_q, const Eigen::Vector3d &cam_odo_t,
    const Eigen::Vector3d &odo_pos, const Eigen::Vector3d &odo_att, const Eigen::Vector2d &observed_p, int flags) const
{
    ceres::CostFunction *costFunction = 0;
            ...
            switch (camera->modelType()) {
                case Camera::KANNALA_BRANDT:
                    costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<EquidistantCamera>, 2, 3>(
                        new ReprojectionError3<EquidistantCamera>(intrinsic_params, cam_odo_q, cam_odo_t, odo_pos,
                                                                  odo_att, observed_p));
                    break;
                case Camera::PINHOLE:
                ...
                case Camera::PINHOLE_FULL:
                    costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<PinholeFullCamera>, 2, 3>(
                        new ReprojectionError3<PinholeFullCamera>(intrinsic_params, cam_odo_q, cam_odo_t, odo_pos,
                                                                  odo_att, observed_p));
                    break;
                case Camera::MEI:
                    costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<CataCamera>, 2, 3>(
                        new ReprojectionError3<CataCamera>(intrinsic_params, cam_odo_q, cam_odo_t, odo_pos, odo_att,
                                                           observed_p));
                    break;
                case Camera::SCARAMUZZA:
                    costFunction = new ceres::AutoDiffCostFunction<ReprojectionError3<OCAMCamera>, 2, 3>(
                        new ReprojectionError3<OCAMCamera>(intrinsic_params, cam_odo_q, cam_odo_t, odo_pos, odo_att,
                                                           observed_p));
                    break;
            }
            break;
...
ceres::CostFunction *CostFunctionFactory::generateCostFunction(const CameraConstPtr &cameraL, const CameraConstPtr &cameraR,
                                                               const Eigen::Vector3d &observed_P,
